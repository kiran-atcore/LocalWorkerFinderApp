    ...
    def get(self, request, *args, **kwargs):
        try:
            with connection.cursor() as cursor:
                cursor.execute("CREATE TABLE IF NOT EXISTS "users_review" ("id" bigint NOT NULL PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, "skill_rating" integer NOT NULL, "performance_rating" integer NOT NULL, "service_quality_rating" integer NOT NULL, "friendly_rating" integer NOT NULL, "cost_efficiency_rating" integer NOT NULL, "overall_rating" double precision NOT NULL, "review_text" text NOT NULL, "created_at" timestamp with time zone NOT NULL, "updated_at" timestamp with time zone NOT NULL, "customer_id" bigint NOT NULL REFERENCES "users_customerprofile" ("id") DEFERRABLE INITIALLY DEFERRED, "worker_id" bigint NOT NULL REFERENCES "users_workerprofile" ("id") DEFERRABLE INITIALLY DEFERRED);")
